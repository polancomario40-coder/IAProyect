...
public class ClaseGastoController : ControllerBase
{
    ...
    [HttpGet("default")]
    public async Task<IActionResult> GetClaseGastoDefault()
    {
        try
        {
            var command = _erpDb.Database.GetDbConnection().CreateCommand();
            ...
            var defaultVar = (result != null && result != DBNull.Value) ? result.ToString() : "01";
            
            return Ok(new { idClaseGasto = defaultVar });
        }
        catch (Exception ex)
        {
            return BadRequest($"Error al consultar clase de gasto default: {ex.Message}");
        }
    }
}
